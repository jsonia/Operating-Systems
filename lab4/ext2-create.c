...
void write_superblock(int fd) {
	...
	superblock.s_blocks_per_group  = 8192;//1024
	superblock.s_frags_per_group   = 8192;//1024
	superblock.s_inodes_per_group  = 128;//128
...
void write_inode_table(int fd) {
    ...
    struct ext2_inode hello_world_node = {0};
    hello_world_node.i_mode = EXT2_S_IFREG
                                  | EXT2_S_IRUSR
                                  | EXT2_S_IWUSR
                                  | EXT2_S_IRGRP
                                  | EXT2_S_IROTH;
    ...
    simlink_node.i_block[0] = 'lleh';
    simlink_node.i_block[1] = 'ow-o';
    simlink_node.i_block[2] = 'dlr';
...
void write_root_dir_block(int fd) {
    ...
    struct ext2_dir_entry hf_entry = {0};
    dir_entry_set(hf_entry, HELLO_WORLD_INO, "hello-world");
    dir_entry_write(hf_entry, fd);
    bytes_remaining -= hf_entry.rec_len;
    
    struct ext2_dir_entry sf_entry = {0};
    dir_entry_set(sf_entry, HELLO_INO, "hello");
    dir_entry_write(sf_entry, fd);
    bytes_remaining -= sf_entry.rec_len;
